...
public class PhotoView extends GLView {
    ...
    public void pause() {
        ...
        hideUndoBar();
        mActivity.removeGestrueChangedListener(null);
    }
    ...
    public void addGestureListener(){
        ...
        mActivity.setGestureChangedListener(new AbstractGalleryActivity.onGestureSensorListener() {
            @Override
            public void onGestureSensorChanged(SensorEvent event) {
                final int action = GestureSensorManger.gesSensorDataToWindow((int) event.values[0]);
                String strAction = "NONE";

                switch (action) {
                    case GestureSensorManger.GESTURE_DOWN:
                    case GestureSensorManger.GESTURE_LEFT:
                        slideToPrevPicture();
                        break;
